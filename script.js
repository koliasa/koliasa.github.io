...
class AHole extends HTMLElement {
  ...
  setLines() {
    ...
    this.lines.forEach((line, i) => {
      ...
      line.forEach((p1, j) => {
        ...
        if (
          !lineIsIn &&
          (ctx.isPointInPath(this.clip.path, p1.x, p1.y) || ctx.isPointInStroke(this.clip.path, p1.x, p1.y))
        ) {
          lineIsIn = true;
        } else if (lineIsIn) {
  ...
  moveParticles() {
    this.particles.forEach((particle) => {
      ...
      if (particle.y < 0) particle.y = this.initParticle().y;
    });
  }
...
document.querySelectorAll(".logo-text span").forEach((letter) => {
